...
const server = new McpServer(
  { name: "sweep", version: "0.0.1" },
  {
    instructions: [
      ...
      "IdentityIQ account, downloads the 3-bureau credit report, extracts every",
      "account, and generates three Affidavit of Truth dispute letters (one per",
      "bureau) ready to print and mail certified.",
      "",
      "AGENT WORKFLOW (recommended):",
      "  1. Call `sweep_pull_iiq_report` with the client's IIQ credentials.",
      "     Returns an HTML file path containing the downloaded credit report.",
      "  2. Call `sweep_extract_accounts` on the HTML path.",
      "     Returns a list of every account-detail listing in the report — each",
      "     with creditor name, per-bureau columns (TransUnion / Experian /",
      "     Equifax), Account #, Account Status, Payment Status, Comments, etc.",
      "     This is raw data — your job is to reason about which items to dispute.",
      "  3. Apply Phillip's dispute-selection rules (see below) to pick which",
      "     listings to include per bureau.",
      "  4. Call `sweep_generate_letters` with the client's personal info and",
      "     your per-bureau dispute selections. Returns 3 PDF paths.",
      "",
      "PHILLIP'S DISPUTE-SELECTION RULES:",
      "  - Each listing in the report is its own potential dispute target. Don't",
      "    merge listings across creditors. Look at EACH bureau column on EACH",
      "    listing independently.",
      "  - Negative indicators (any of these in Account Status / Payment Status /",
      "    Account Type - Detail / Comments) → dispute:",
      "      • Collection / Charged off / Chargeoff",
      "      • Derogatory",
      "      • Foreclosure / Repossession / Bankruptcy",
      "      • Late 30/60/90/120/150 Days",
      "  - Exception: lates on OPEN accounts → SKIP (don't dispute open-account",
      "    late history; only dispute closed-account lates).",
      "  - Self-reported items (TELECOM SELFREPORTED, INSURANCE SELFREPORTED,",
      "    utility, DPT ED/AIDV, ATLAS) → only dispute if Account Status or",
      "    Payment Status explicitly shows negative (don't trust Account Type -",
      "    Detail or Comments for these).",
      "  - Do NOT include inquiries in the dispute letters.",
      "  - Leave creditor names exactly as IIQ shows them per bureau (don't",
      "    canonicalize across bureaus — each bureau has its own naming).",
      "  - Per-bureau independence: an account that's negative on Experian but",
      "    positive on TransUnion gets disputed on Experian's letter only.",
      "",
